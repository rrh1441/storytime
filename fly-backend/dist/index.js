...
import { generateSpeech, VOICES, } from "./services/tts.js"; // 👈  .js ext
/*───────────────────────────────────────────────────────────────────────────
  Environment
───────────────────────────────────────────────────────────────────────────*/
const { PORT = "8080", SUPABASE_URL, SUPABASE_ANON_KEY, OPENAI_API_KEY, } = process.env;
if (!SUPABASE_URL)
...
app.post("/tts", async (req, res) => {
    try {
        ...
        // 1. Generate speech
        const { mp3Buffer, contentType } = await generateSpeech(text, voice, language);
