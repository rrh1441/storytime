...
const estimateWordCount = (m: number) => Math.max(150, Math.round(m * wordsPerMinute));
...
export async function generateStoryAction(
  ...
  const { theme, length, language, mainCharacter, educationalFocus, additionalInstructions } = parsed.data;
  ...
  try {
    ...
    const story  = titleMatch ? raw.replace(new RegExp(`${TITLE_MARK}.*`, 'm'), '').trim() : raw.trim();
    ...
    console.error('[generateStoryAction]', e);
    return { error: e.message ?? 'Story generation failed.' };
  }
}
...
export async function generateTtsAction(
  ...
  try {
    ...
    const { error } = await supabaseService.storage.from(bucket)
    ...
    console.error('[generateTtsAction]', e);
